...
#部署dist
ansible -i ${ScriptDir}/DeployDistHost DeployList -m shell -a '/bin/bash /root/.guozhichao/DeployDist.sh removes=/root/.guozhichao/DeployDist.sh'
[ $? -ne 0 ] && echo "部署dist报错，请检查。" && exit
#刷新CDN缓存
[ -f /root/aliy/refresh_cdn.py ] && python3 /root/aliy/refresh_cdn.py https://storage.titannet.io/
