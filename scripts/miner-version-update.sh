...
do
   ...
   stop_miner() {
      ...
      which lotus-miner > /dev/null && [ $? -ne 0 ] && echo "lotus-miner程序不存在，请检查。" && exit
      echo "Lotus的API信息:"
      ...
      if ps -aux |grep -v grep |grep -w lotusminer-${stopkey} ;then
         if lotus-miner auth api-info --perm admin |grep -w "$MinerListenPort" ;then
            ...
            lotus-miner stop && echo "lotusminer-${stopkey}程序停止中......"
         fi
      else
   ...
   miner_version_update() {
      ...
      while :
      do
            if ps -aux |grep -v grep |grep -w $updatekey 2>/dev/null ;then
                    sleep 1s
                    continue
            else
   ...
   start_miner() {
      ...
      while :
      do
         if ps -aux |grep -v grep |grep -w lotusminer-${startkey} ;then
                 sleep 1s
                 continue
         else
		 echo "source /opt/raid0/lotusminer-${startkey}/profile"
                 echo "bash /opt/raid0/lotusminer-${startkey}/start_lotusminer.sh"
                 source /opt/raid0/lotusminer-${startkey}/profile
                 bash /opt/raid0/lotusminer-${startkey}/start_lotusminer.sh && echo "Lotusminer-${startkey}程序启动中......"
		 break
         fi
      done
   }
   menu() {
   	...
   	echo "2: 查看miner进程"
   	echo "3: 查看miner版本"
	echo "4: Stop miner(说明：建议先停调度，再停抽查（要留充足的时间，为保证下一个抽查的窗口正常上链，最后停爆块)"
	echo "5: Miner版本更新"
	echo "6: Start miner(说明：建议先启爆块，再启调度，最后是启动抽查的时候，视抽查的时间开始启动，否则可能会出现抽查重做的问题，会拖慢抽查的时间导致不能正常上链。尤其是云库这样的大节点会出现窗口重做的现象，请谨慎操作)"
   	echo "7: 退出脚本"
   }
   menu
   read -p "请选择:" choose
