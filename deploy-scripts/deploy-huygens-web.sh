...
cd $git_dir/new-testing-official-website
...
#拉取代码
echoGreen "开始拉去代码"
cd $git_dir/new-testing-official-website  && rm -rf dist && git checkout . && git pull &&  git checkout master
if [ $? -eq 0 ];then
        echoGreen "拉取代码成功......."
else
        echoRed "代码拉取失败...."
...
if [ $? -eq 0 ];then
    ...
    echoGreen "生成镜像"
    echoGreen "`pwd`"
    echoGreen "docker build . -t registry.cn-hongkong.aliyuncs.com/nft/$project_name:$time_build  -f  ${project_name}dockerfile"
