...
project_name='titan-minefi-dapp'
...
echoYellow "cd 到工作目录 $git_dir/minefi-dapp"
cd $git_dir/minefi-dapp
...
#拉取代码
echoGreen "开始拉去代码"
cd $git_dir/minefi-dapp && rm -rf dist && git checkout . && git pull &&  git checkout ID9527
if [ $? -eq 0 ];then
        echoGreen "拉取代码成功......."
else
        echoRed "代码拉取失败...."
