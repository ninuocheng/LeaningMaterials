...
echoYellow "cd 到工作目录 $git_dir/music-nft-h5-active "
cd $git_dir/music-nft-h5-active
  ...
  - Image: registry.cn-hongkong.aliyuncs.com/nft/$project_name:$time_build
    ...
    Ports:
      - Port: 80
        ExposePort: 80
    Env:
      VITE_REQUEST_BASE_URL: "https://api.bdnft.com"
EOF
...
#拉取代码
echoGreen "开始拉去代码"
cd $git_dir/music-nft-h5-active && rm -rf dist && git checkout . && git pull &&  git checkout main
if [ $? -eq 0 ];then
        echoGreen "拉取代码成功......."
else
        echoRed "代码拉取失败...."
