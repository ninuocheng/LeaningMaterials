...
[ "$InitialNub" -eq 0 ] &&  echo "准备的文件$ScriptDir/$ImportDealFile要导入的订单数量为0，请检查。" && exit 3
#导入订单的日志
...
[ $DealsMapNub -gt $MinDealsMapNub ] && echo "DealsMap的数量大于给定的最小值$MinDealsMapNub，无需导入订单。" && exit 11
#正在封装的PC1数量
PC1SealingJobsNub=`lotus-miner sealing jobs |grep -wic pc1`
[ $PC1SealingJobsNub -gt $MaxPC1SealingJobs ] && echo "目前正在封装的PC1数量: ${PC1SealingJobsNub}，处于饱和状态，无需导入订单。" && exit 6
...
sleep 30s
[ "$PauseFinalizeWorkersNub" -eq "$FinalizeWorkersNub" ] && echo "已暂停落盘操作"
...
do
	...
	#调用管道会打开一个新的bash，所以要执行多次exit才能退出该脚本，否则会一直循环.....
        [ "$PauseFinalizeWorkersNub" -eq 0 ] && echo "已成功恢复落盘"  && exit 7 && exit 8 && exit 9
     else
